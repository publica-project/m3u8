...
func TestMediaPlaylistWithAdobeTag(t *testing.T) {
	...
	t.Run("media-playlist-with-cue-out.m3u8", func(t *testing.T) {
		...
		expect := map[int]*SCTE{
			2: {Syntax: ADOBE, CueType: SCTE35Cue_Start, Time: 60, ID: ""},
			18: {Syntax: SCTE35_OATCLS, CueType: SCTE35Cue_End, ID: ""},
		}
		for i := 0; i < int(pp.Count()); i++ {
			if expect[i] != nil {
				if !reflect.DeepEqual(pp.Segments[i].SCTE, expect[i]) {
	...
	t.Run("media-playlist-with-adobe-cue-out-in.m3u8", func(t *testing.T) {
		f, err := os.Open("sample-playlists/media-playlist-with-adobe-cue-out-in.m3u8")
		if err != nil {
			t.Fatal(err)
		}
		p, _, err := DecodeFrom(bufio.NewReader(f), true)
		...
		expect := map[int]*SCTE{
			0: {Syntax: ADOBE, CueType: SCTE35Cue_Start, Time: 60, ID: ""},
		}
		for i := 0; i < int(pp.Count()); i++ {
			if expect[i] != nil {
				if !reflect.DeepEqual(pp.Segments[i].SCTE, expect[i]) {
...
func TestDecodeMediaPlaylistAsset(t *testing.T) {
	f, err := os.Open("sample-playlists/media-playlist-with-asset.m3u8")
	if err != nil {
		t.Fatal(err)
	}
	p, listType, err := DecodeFrom(bufio.NewReader(f), true)
	...
	test := func(name, value string) {
		if pp.Segments[2].Asset[name] != value {
			t.Errorf(`Asset "%s" != "%s"`, name, value)
		}
	}

	test("DURATION", "120")
	test("CONTENT_TITLE", "Awesome%20Show")
	test("CONTENT_ID", "123")
}
